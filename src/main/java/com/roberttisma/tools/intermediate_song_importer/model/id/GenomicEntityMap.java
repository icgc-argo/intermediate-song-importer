...
public class GenomicEntityMap {
  ...
  private Stream<GenomicEntity> streamEntity(GenomicTypes genomicType, Function<SubmitterIdAgg, Set<String>> getter, Entry<String, SubmitterIdAgg>entry){
    val submitterIdAgg = entry.getValue();
    val studyId = entry.getKey();
    return getter.apply(submitterIdAgg).parallelStream()
        .map(x -> GenomicEntity.builder()
            .submitterId(x)
