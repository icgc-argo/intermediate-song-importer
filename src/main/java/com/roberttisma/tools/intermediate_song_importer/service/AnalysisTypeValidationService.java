...
public class AnalysisTypeValidationService {
  ...
  public List<Report> validateLatestAnalysisType(@NonNull Collection<Path> jsonFiles) {
    ...
    val currentAnalysisTypeIdMap =
        jsonFiles.stream().collect(groupingBy(Payloads::parseAnalysisTypeId));

    // Get names of analysisTypes
    val names =
        currentAnalysisTypeIdMap.keySet().stream()
            .map(AnalysisTypeId::getName)
            .collect(toUnmodifiableSet());

    // Find the latest analysisType versions for those names
    val latestAnalysisTypeIdMap =
        targetSongService.getLatestAnalysisTypeIds(names).stream()
    ...
    for (val name : latestAnalysisTypeIdMap.keySet()) {
      ...
      val currentErrors =
          currentAnalysisTypeIdMap.keySet().stream()
              ...
              .map(
                  x ->
                      AnalysisTypeValidationErrorReport.builder()
                          .name(x.getName())
      ...
      if (!currentErrors.isEmpty()) {
        log.error(
            "[ANALYSIS_TYPE_VALIDATION_ERROR]: Several files with the analysisType name '{}' do not have the latest version '{}'",
            name,
            latestVersion);
      }
