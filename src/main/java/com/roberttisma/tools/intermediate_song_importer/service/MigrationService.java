...
public class MigrationService implements Closeable {
  ...
  public void migrate(@NonNull Path jsonFile) {
    try{
      checkImporter(jsonFile.toString().endsWith(".json"),
          "The file '%s' is not a json file", jsonFile.toString());
      ...
      val message = format("[PROCESSING_ERROR] filename='%s' errorType='%s':  '%s",
  ...
  private void updateAnalysisFiles(Analysis source, Analysis target){
    val targetMap = target.getFiles().stream().collect(toMap(FileDTO::getFileName, FileDTO::getObjectId));
    source.getFiles()
        .stream()
        .map(s -> {
          checkImporter(targetMap.containsKey(s.getFileName()),
              "No matching filename '%s' for source analysisId '%s' and target analysisId '%s'",
              s.getFileName(), s.getAnalysisId(), target.getAnalysisId());
