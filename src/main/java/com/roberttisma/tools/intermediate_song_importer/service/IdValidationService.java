...
public class IdValidationService {
  ...
  public Stream<GenomicEntity> validate(@NonNull Collection<Path> jsonFiles) {
    ...
    return genomicEntityMap.streamGenomicEntities()
        .filter(this::isNonExistent);
  }
  ...
  private boolean isNonExistent(GenomicEntity genomicEntity){
    ...
    switch (genomicType){
    case DONOR:
      return idService.getDonorId(genomicEntity.getStudyId(),genomicEntity.getSubmitterId()).isEmpty();
    case SPECIMEN:
      return idService.getSpecimenId(genomicEntity.getStudyId(),genomicEntity.getSubmitterId()).isEmpty();
    case SAMPLE:
      return idService.getSampleId(genomicEntity.getStudyId(),genomicEntity.getSubmitterId()).isEmpty();
    default:
