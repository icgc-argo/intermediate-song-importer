...
public class SourceSongService {
  ...
  public List<FileDTO> getSourceAnalysisFiles(@NonNull Path payloadFile) {
    return processSourceData(payloadFile)
        .stream()
  ...
  private static Set<String> parseLegacyAnalysisIds(Path file) {
    ...
    checkImporter(fileNamesMissingLegacyAnalysisId.isEmpty(),
        "The following filenames for payload file '%s' are missing the info field '%s'",
        file.toString(), COMMA_SPACE.join(fileNamesMissingLegacyAnalysisId) );
  ...
  private static Set<FileDTO> parseFiles(Path file){
    ...
    checkImporter(payload.has("files"),
        "The file '%s' does not contain the field 'files'", file);
    return newArrayList(payload.path("files").iterator())
        .stream()
